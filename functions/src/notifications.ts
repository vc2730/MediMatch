...
interface PatientNotificationParams {
  patientId: string
  patientName: string
  patientEmail?: string
  patientPhone?: string
...
interface DoctorNotificationParams {
  doctorId: string
  doctorName: string
  doctorEmail?: string
  doctorPhone?: string
...
export async function sendDoctorNotification(
  params: DoctorNotificationParams
): Promise<void> {
  const {
    doctorId,
    doctorName,
 ...
 */
export async function sendCareTeamNotifications(params: {
  matchId: string
  teamMembers: Array<{
    role: string
    name: string
 ...
 */
function generateEmailHTML(params: PatientNotificationParams): string {
  return `
    <!DOCTYPE html>
    <html>
    <head>
 ...
 */
function generateDoctorEmailHTML(params: DoctorNotificationParams): string {
  const urgencyColor =
    params.urgencyLevel >= 8 ? '#dc2626' : params.urgencyLevel >= 5 ? '#f59e0b' : '#2563eb'
