...
import { sendCareTeamNotifications } from './notifications'

const db = getFirestore()

interface CoordinationPlanParams {
  matchId: string
  patientId: string
  doctorId: string
...
interface Resource {
  resource: string
  status: 'allocated' | 'staged' | 'standby' | 'on-demand'
  room: string
}

interface CommunicationItem {
  channel: 'SMS' | 'Email' | 'Pager' | 'EHR Alert'
  recipient: string
  message: string
  timing: 'immediate' | 'on-arrival' | 'scheduled'
}
...
interface CoordinationPlan {
  ...
  careTeamAssignments: CareTeamMember[]
 ...
 */
export const generateCoordinationPlan = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
  ...
  await matchRef.update({
    ...
    coordinationPlanGeneratedAt: FieldValue.serverTimestamp()
  })

  // Trigger care team notifications
  if (plan.careTeamAssignments.length > 0) {
    await sendCareTeamNotifications({
      matchId,
...
async function buildCoordinationPlan(params: {
  ...
  const careTeamAssignments: CareTeamMember[] = [
    {
  ...
  // Resource allocation
  const resourceAllocation: Resource[] = [
    {
  ...
  // Communication plan
  const communicationPlan: CommunicationItem[] = [
    {
      channel: 'SMS',
      recipient: 'Patient',
 ...
 */
function identifyBottlenecks(match: any, appointment: any, patient: any): Bottleneck[] {
 ...
 */
function generateOptimizations(patient: any, match: any): Optimization[] {
  ...
  if (equityScore > 70) {
    optimizations.push({
      ...
      rationale:
        'Patient has significant barriers to care - may benefit from care navigation support',
 ...
 */
function getSpecialistRole(specialty: string): string {
 ...
 */
export const getCoordinationPlan = onCall({ cors: true }, async (request) => {
  const { matchId } = request.data
  ...
  if (!planSnap.exists) {
    throw new HttpsError('not-found', 'Coordination plan not found')
  }
